...
export async function POST(request: NextRequest) {
  try {
    ...
    if (event === 'payment.captured') {
      ...
      // Verify amount matches the rate for the student's academic year (₹100 for 1st Year, ₹200 for 2nd/3rd/4th Year)
      ...
      const isWebhookAmountValid = amount === expectedAmount || amount === Number(payment.amount) || (amount === 15000 && resolvedYear !== '1st Year');
      if (!isWebhookAmountValid || currency !== 'INR') {
